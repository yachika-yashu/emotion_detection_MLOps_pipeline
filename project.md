...
- **Limitation:** TF-IDF fundamentally loses contextual word relationships and sequential meaning compared to dense vector embeddings.
- **Improvement:** Upgrade the feature engineering stage to leverage pretrained Transformer embeddings (e.g., HuggingFace `sentence-transformers`).
- **Limitation:** Text preprocessing aggressively removes all punctuation and numbers, which often contain heavy emotional weight in social media data (e.g., emojis, exclamation marks).
- **Improvement:** Implement an emoji-aware tokenizer before stripping specific punctuation characters.
- **Limitation:** Models and datasets are saved locally rather than in a cloud artifact registry.
- **Improvement:** Configure `dvc remote add` to sync artifacts to an AWS S3 bucket using the pre-written `sync_data_up` Makefile command.
...
## 📊 Metrics
- **Lines of code:** ~500+ (excluding notebooks)
- **Files:** 28 (Core source code and configurations)
- **Dependencies:** 225 (Python packages via `requirements.txt`)
